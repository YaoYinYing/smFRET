...
good = fltarr(2,10000)
back = fltarr(10000)
; back2 = fltarr(10000) ; no ALEX, one blank
...
while EOF(2) ne 1 do begin
	...
	good(1,no_good) = y
	back(no_good) = b
	; back2(no_good) = b2 ; no ALEX, one blank
	no_good = no_good + 1
endwhile
...
for i = 0, film_l - 1 do begin
	...
	for j = 0, no_good - 1 do begin
		...
		apeak = g_peaks(whc_gpk(j,0),whc_gpk(j,1),*,*) * median(ave_arr(flgd(0,j)-7:flgd(0,j)+7,flgd(1,j)-7:flgd(1,j)+7))
